...
function Basket({setGetTheme, getTheme }) {
    ...
    return (
    <div className="relative w-fit cursor-pointer" onClick={shopHandler}>
        <svg xmlns="http://www.w3.org/2000/svg" fill={getTheme === 'dark' ? 'white' : 'black'} viewBox="0 0 24 24" strokeWidth={1.5} stroke="black" className="size-6 select-none dark:text-white">
          <path d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
        </svg>
        <span className="size-4 absolute top-4 left-3 flex justify-center items-center text-base pt-1 bg-primaryColor text-white rounded-full">{countProducts == 0 ? "0" : countProducts} </span>
        <div className={`md:w-96 w-72 ${selectQuantity == 0 ? 'h-60' : 'h-80'} overflow-y-auto .scrollbar-thumb scrollbar-track scrollbar ${open ? 'visible' : 'hidden'} absolute top-[3rem] left-0 z-[10000] rounded-xl text-black dark:bg-black dark:text-white`}>
          {selectQuantity == 0 ?
